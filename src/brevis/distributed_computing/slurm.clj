...
(defn gen-config
  ...
  (let [out-str (str
                  ...
                  " :run-number $SLURM_ARRAY_TASK_ID"
                  "\n"
                  #_" > "
...
(defn launch-config
  ...
  (let [command (str "sbatch " configFile " --array 1-" (str numruns) " --job-name " expName)];  used to include optArgs, removed because it returned {}
...
sed -n -e \"$SLURM_ARRAY_TASK_ID p\" " (str destination expName "/" command-filename) " | sh")))
     ...
     (println "All runs submitted.")))
>>>>>>> branch 'slurm' of git@github.com:kephale/brevis.git
