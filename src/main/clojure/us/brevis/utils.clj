(ns us.brevis.utils
  ...
  (:import [us.brevis Engine BrObject]
           [java.io FileInputStream FileOutputStream
            ObjectInputStream ObjectOutputStream])
  (:use [us.brevis globals]
...
(defn add-object
  ...
  [^BrObject obj]
  (.addObject ^us.brevis.Engine @*java-engine* (get-uid obj) ^BrObject obj)
  obj)

(defn del-object
  "Add an object to the current world."
  [^BrObject obj]
  (.deleteObject ^us.brevis.Engine @*java-engine* (get-uid obj)))

(defn add-update-handler
  "Associate an update function with a type."
  [type handler-fn]
  (let [uh (proxy [us.brevis.UpdateHandler] []
             (update [#^us.brevis.Engine engine #^Long uid #^Double dt]
               (let [obj (.getObject engine uid)]
                 (handler-fn obj))))]
    (.addUpdateHandler ^us.brevis.Engine @*java-engine* (str (name type)) uh)))

(defn add-global-update-handler
  "Add a global update handler with specified priority."
  [priority handler-fn]
  (let [gh (proxy [us.brevis.GlobalUpdateHandler] []
             (update [#^us.brevis.Engine engine]
               (handler-fn)))]
    (.addGlobalUpdateHandler ^us.brevis.Engine @*java-engine* priority gh)))
...
(defn load-simulation-state
  ...
  (try 
    (let [fis (FileInputStream. filename)
          ...
          engine (cast Engine (.readObject in))]
